...
async function getLatestModifyDateByFile(): Promise<Date> {
    // get last modified date of the zotero library by file modification date
    const preferences: Preferences = getPreferenceValues();
    const f_path = resolveHome(preferences.zotero_path);

    return (await stat(f_path)).mtime;
}

async function getLatestModifyDate(): Promise<Date> {
  // get last modified data of the zotero by querying the database (deprecated)
...
export const searchResources = async (q: string): Promise<RefData[]> => {
  ...
  async function readCache(): Promise<RefData[]> {
    ...
    //   return fData.data;
    // } else {
    const latest = await getLatestModifyDateByFile();
